...
nmap -sn -D 192.168.0.114,192.168.0.115,192.168.0.116,192.168.0.117,192.168.0.118 --spoof-mac 00:50:56:9D:E3:F9 -n "$TARGET_NETWORK" |
grep "Nmap scan report for" | awk '{print $5}' | sort -u > "$LIVE_HOSTS"
...
if [ -s "$LIVE_HOSTS" ]; then
	...
	xargs -a "$LIVE_HOSTS" -P 5 -I {} nmap -n -p 1-1000,3000-4000 -D 192.168.0.114,192.168.0.115,192.168.0.116,192.168.0.117,192.168.0.118 --spoof-mac 00:50:56:9D:3C:C6 -sS -sV -O --host-timeout 5m {} -oN "$NMAP_OUTPUT_DIR/nmap_scan_{}.txt"
