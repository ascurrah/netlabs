...
fetch_robots() {
  ...
  # FIX: combine local with assignment so set -e does not trigger on grep
  # returning exit code 1 (no match). 'local' always returns 0, masking
  # the non-zero exit from the subshell.
  local user="$(echo "$body" | grep -oiE 'User(-name)?[[:space:]]*:[[:space:]]*[a-zA-Z0-9_-]+' \
                | awk -F: '{print $NF}' | tr -d ' ' | head -1 || true)"

  if [[ -z "$user" ]]; then
    # FIX: append '|| true' so a no-match grep does not abort the script
    user="$(echo "$body" | grep -vE '^#|^$|User-agent|Disallow|Allow' \
            | grep -oE '[a-zA-Z][a-zA-Z0-9_-]{2,}' | head -1 || true)"
  fi
...
run_hydra() {
  ...
  local hydra_out
  # FIX: append '|| true' so hydra's non-zero exit on failure does not
  ...
  # FIX: combine local with assignment for the same set -e reason
  local pass="$(echo "$hydra_out" \
                | grep -oP '(?<=password: )\S+' \
                | head -1 || true)"
...
run_ssh_enum() {
  ...
  # FIX: '|| true' prevents set -e aborting if find returns non-zero
  local find1_out
  find1_out="$(
