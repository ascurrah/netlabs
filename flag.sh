...
step3_nmap_top() {
  ...
  # Run nmap but do not let a non-zero exit code terminate the script
  NMAP_OUT="$(sudo nmap --top-ports 1024 "$TARGET_IP" 2>&1 || true)"
  echo "$NMAP_OUT"
...
# ── Step 7 — curl robots.txt ──────────────────────────────
step7_curl_robots() {
  step 7 "curl — robots.txt"

  local url="http://${TARGET_IP}:${HTTP_PORT}/robots.txt"
...
REMOTE
  ...
  ssh \
    ...
    -p "$SSH_PORT" \
    "${SSH_USER}@${TARGET_IP}" \
    "bash -c '$remote_cmds'; exec bash --login" \
    || true
}
